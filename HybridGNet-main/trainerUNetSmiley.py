...
# from utils.smileyDataLoader import LandmarksDataset, ToTensorSeg1, ToTensorSegLH, Rescale, RandomScale, AugColor, Rotate
from utils.smileyDataLoaderMoreEyes import LandmarksDataset, ToTensorSeg1, ToTensorSegLH, Rescale, RandomScale, AugColor, Rotate
...
def trainer(train_dataset, val_dataset, model, config):
    ...
    for epoch in range(config['epochs']):
        ...
        print("trainloader", train_loader.__len__())
        ...
        val_loss_avg_compare_new.append(0)
        ...
        with torch.no_grad():
            for sample_batched in val_loader:                
                ...
                # print("image and target", image.shape, target.shape)

                out = model(image)
                seg = torch.argmax(out[0,:,:,:], axis = 0).cpu().numpy()
                dcl, dch = evalImageMetrics(seg, target[0,:,:])
                
                val_loss_avg_compare_new[-1] += (dice_loss(out, torch.tensor(target).to(device)) + ce_loss(out, torch.tensor(target).to(device))).item()

                val_loss_avg_compare[-1] += dice_loss(out, torch.tensor(target).to(device)).item()
        ...
        val_loss_avg_compare_new[-1] /= num_batches
        ...
        writer.add_scalars('Loss same dice loss', {'Train': train_loss_avg[-1], 'Val': val_loss_avg_compare_new[-1]}, epoch) 
        ...
        scheduler.step()
        print('Epoch time: %f' % (time.time() - starttime))
        # save validation and training loss
    epochs = range(1, config['epochs'] + 1)

    plt.plot(epochs, train_loss_avg, label = 'Train loss dice - ', color ='blue')
    plt.plot(val_loss_avg_compare_new, label = 'Val loss', color = 'orange')
    plt.legend()
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.grid('True')
    plt.title('Train and Val in dice + ce_loss Loss U-Net')
...
if __name__ == "__main__":
    ...
    # else:
    # train_path = "../../generateData/data/maskTrain"
    # val_path = "../../generateData/data/maskVal" 
    ...
    config['name'] = "MoreEyes500epochsnoisyTrainValLossUNet"
