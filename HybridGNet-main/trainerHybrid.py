...
def hd_landmarks(out, label, size = (512, 512)):
    shape = size
...
def trainer(train_dataset, val_dataset, model, config):
    ...
    for epoch in range(config['epochs']):
        ...
        with torch.no_grad():
            for sample_batched in val_loader:
                ...
                val_loss_avg_compare[-1] += loss.item()
                num_batches += 1   
                loss_rec = 0
    ...
    plt.ylabel('MSE Loss')
    plt.grid('True')
    plt.title('Train and Val Loss ' + model.__class__.__name__)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inputsize", default = [1024,1024], type=int)
    ...
    # config['name'] = "JSRTLungDataHybrid500NoSkipValLoss"
    config['name'] = "JSRTLungDataHybrid500doubleSkipValLoss"
...
#     config['val_batch_size'] = 1
#     config['weight_decay'] = 1e-5
#     config['name'] = "JSRTLungDataHybrid5001skipValLoss"
