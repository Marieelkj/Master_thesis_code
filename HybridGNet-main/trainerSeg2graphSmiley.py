...
from utils.utilsSmiley import scipy_to_torch_sparse, genMatrixesLH, genMatrixesLungs
from utils.smileyGraphDataLoader import LandmarksDataset, ToTensorSegHeart, ToTensorSegLungs, RandomScale, Rotate, RandomMove
...
def trainer(train_dataset, val_dataset, model, config):
    ...
    for epoch in range(config['epochs']):
        ...
        print('Epoch [%d / %d] train average reconstruction error: %f' % (epoch+1, config['epochs'], train_rec_loss_avg[-1]*160*128))
        ...
        scheduler.step()
        print('Epoch took: %f' % (time.time() - starttime))
...
if __name__ == "__main__":
    ...
    config['name'] = 'seg2noisy500epocsSmileyHybrid'
    ...
    config['val_batch_size'] = 1
    config['weight_decay'] = 1e-5
    config['name'] = 'seg2noisy500epocsSmiley1Skip'
    ...
    config['val_batch_size'] = 1
    config['weight_decay'] = 1e-5
    config['name'] = 'seg2noisy500epocsSmileyDoubleSkip'
