...
def trainer(train_dataset, val_dataset, model, config):
    ...
    for epoch in range(config['epochs']):
        ...
        for sample_batched in train_loader:
            ...
            # print("image and target", image.dtype, target.dtype)
            # image and target torch.float32 torch.int64
            out = model(image)
        ...
        model.eval()
        val_loss_avg_copare.append(0)
        val_loss_avg_copare_new.append(0)
        ...
        with torch.no_grad():
            for sample_batched in val_loader:                
                ...
                num_batches += 1   
                loss_rec = 0
                val_loss_avg_copare[-1] += dice_loss(out, torch.tensor(target).to(device)).item()
                val_loss_avg_copare_new[-1] += (dice_loss(out, torch.tensor(target).to(device)) + ce_loss(out, torch.tensor(target).to(device))).item()
        print('')
        
        val_loss_avg_copare[-1] /= num_batches
        val_loss_avg_copare_new[-1] /= num_batches
        ...
        writer.add_scalars('Loss', {'Train': train_loss_avg[-1], 'Val': val_loss_avg_copare[-1]}, epoch)
        writer.add_scalars('Loss same train and val', {'Train': train_loss_avg[-1], 'Val': val_loss_avg_copare_new[-1]}, epoch)
    ...
    plt.plot(val_loss_avg_copare_new, label = 'Val loss', color = 'orange')
    plt.legend()
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.grid('True')
    ...
    plt.grid('True')
    plt.yscale('log')
    plt.title('Train and Val loss in dice + ce_loss U-Net')
...
if __name__ == "__main__":
    ...
    config['name'] = "lungsUnet500EpochsWithValLossCompCorrect"
