...
class EncoderConv(nn.Module):
    def __init__(self, latents = 64, hw = 32):
        ...
        self.fc_mu = nn.Linear(in_features=self.size[4]*hw[0]*hw[1], out_features=self.latents)
        self.fc_logvar = nn.Linear(in_features=self.size[4]*hw[0]*hw[1], out_features=self.latents)
...
class Hybrid(nn.Module):
    def __init__(self, config, downsample_matrices, upsample_matrices, adjacency_matrices):
        ...
        hw = [config['inputsize'][0] // 32, config['inputsize'][1] // 32]
