...
from utils.utilsSmileyMoreEyes import scipy_to_torch_sparse, genMatrixes, genMatrixesLH
from utils.smileyDataLoaderMoreEyes import LandmarksDataset, ToTensor, ToTensorLH, Rescale, RandomScale, AugColor, Rotate
...
def hd_landmarks(out, label, size = (160, 128)):
    shape = size

    target = np.round(label.cpu().numpy()*size[0]).astype('int32').clip(0, size[1] - 1)
    pred = np.round(out.cpu().numpy()*size[0]).astype('int32').clip(0, size[1] - 1)
    
    d_lungs = hd_land(target[:12,:], pred[:12,:], shape)
    d_heart = hd_land(target[12:24,:], pred[12:24,:], shape)
    
    if target.shape[0] > 24:
        d_cla =  hd_land(target[24:,:], pred[24:,:], shape)
...
def trainer(train_dataset, val_dataset, model, config):
    ...
    train_rec_loss_avg = []
    val_loss_avg = []
    val_loss_avg_compare = []
    val_hd_avg = []
       
    tensorboard = "Training1"
    ...
    for epoch in range(config['epochs']):
        ...
        print('Epoch [%d / %d] train average reconstruction error: %f' % (epoch+1, config['epochs'], train_rec_loss_avg[-1]*200*200))

        num_batches = 0

        model.eval()
        val_loss_avg.append(0)
        val_loss_avg_compare.append(0)
        val_hd_avg.append(0)
        ...
        with torch.no_grad():
            for sample_batched in val_loader:
                ...
                val_loss_avg_compare[-1] += loss_rec.item()
                num_batches += 1   
                loss_rec = 0

        val_loss_avg[-1] /= num_batches
        val_loss_avg_compare[-1] /= num_batches
        val_hd_avg[-1] /= num_batches
        
        print('Epoch [%d / %d] validation average reconstruction error: %f' % (epoch+1, config['epochs'], val_loss_avg[-1] * 160 * 128))

        writer.add_scalar('Train/Loss', train_loss_avg[-1], epoch)
        writer.add_scalars('Loss', {'Train': train_loss_avg[-1], 'Val': val_loss_avg_compare[-1]}, epoch)

        writer.add_scalar('Train/Loss kld', train_kld_loss_avg[-1], epoch)
        writer.add_scalar('Train/MSE', train_rec_loss_avg[-1] * 160 * 128, epoch)
        
        writer.add_scalar('Validation/MSE', val_loss_avg[-1]  * 160 * 128, epoch)
    ...
    plt.plot(val_loss_avg_compare, label = 'Val loss', color = 'orange')
    plt.legend()
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.grid('True')
    plt.title('Training Loss and Validation Loss ' + model.__class__.__name__)
    plt.savefig(os.path.join('Loss', 'Loss' + config['name'] + '.png'))
    plt.close()
    plt.plot(epochs, train_loss_avg, label = 'Train loss', color ='blue')
    plt.plot(val_loss_avg_compare, label = 'Val loss', color = 'orange')
    plt.legend()
    ...
    plt.grid('True')
    plt.yscale('log')
    plt.title('Training Loss and Validation Loss Log' + model.__class__.__name__)
    plt.savefig(os.path.join('Loss', 'Loss log' + config['name'] + '.png'))
    plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inputsize", default = [320,256], type=int, nargs=2)
    ...
    if config['allOrgans']:
        ...
        print('A', A.shape, 'AD', AD.shape, 'D', D.shape, 'U', U.shape)
        ToTensor = ToTensorLH
    ...
    print("hellor", D_t[0].shape, U_t[0].shape, A_t[0].shape)
    ...
    config['name'] = "moreEyes500EpocsHybridGNetNoAugEyeValLoss"
...
#   Defining what model to use: Default 'HybridGNet'
    ...
    config['val_batch_size'] = 1
    config['weight_decay'] = 1e-5
    config['name'] = "moreEyes500Epocs1SkipNoAugLayer6ValLoss"
    ...
    ############################################################################
      # Defining what model to use: Default 'HybridGNet'
    parser.set_defaults(skip=False)
    parser.set_defaults(doubleskip=True)
    parser.set_defaults(extended=False)
    parser.set_defaults(pooling=True)
    ...
    config['val_batch_size'] = 1
    config['weight_decay'] = 1e-5
    config['name'] = "moreEyes500EpocsDoubleSkipNoAugLayer65ValLoss"
