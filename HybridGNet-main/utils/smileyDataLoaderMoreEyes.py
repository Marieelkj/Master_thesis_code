...
import cv2 
from .smileyGraphDataLoaderMoreEyes import getSeg, getHeart
...
class LandmarksDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #image = io.imread(img_name).astype('float') /255
        # Normalize to [0, 1]
...
class ToTensorLungs(object):
    ...
    def __call__(self, sample):
        ...
        image = image.transpose((2, 0, 1))
        landmarks = landmarks.reshape(-1, 2) / size
        landmarks = np.clip(landmarks, 0, 1)[:12]
...
class ToTensorLH(object):
    ...
    def __call__(self, sample):
        ...
        landmarks = np.clip(landmarks, 0, 1)[:24]
