...
class RandomMove(object):
    ...
    def __call__(self, sample):
        ...
        if mx > 320:
            landmarks[:, 0] = 320 * (landmarks[:, 0] / mx)
            mx = 320
        if my > 256:
            landmarks[:, 1] = 256 * (landmarks[:, 1] / my)
            my = 256
                    
        move_x = np.random.uniform(-nx/3, (320-mx)/3)
        move_y = np.random.uniform(-ny/3, (256-my)/3)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        landmarks = landmarks.reshape(-1, 2) / 320 # maybe 256 here also?
        landmarks = np.clip(landmarks, 0, 1)
